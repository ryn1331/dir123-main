...
export default function AdminSettings() {
  ...
  const handleEmailChange = async () => {
    ...
    setAuthSaving(true);
    const { data: { user } } = await supabase.auth.getUser();
    if (!user?.email) { setAuthSaving(false); return toast.error("Session invalide"); }
    const { error: signErr } = await supabase.auth.signInWithPassword({ email: user.email, password: currentPassword });
    if (signErr) { setAuthSaving(false); return toast.error("Mot de passe incorrect"); }
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="bg-card border border-border rounded-xl p-5 space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Messenger" settingKey="whatsapp" maxLength={200} />
          </div>
      ...
      <div className="bg-card border border-border rounded-xl p-5 space-y-4">
        ...
        <p className="text-xs text-muted-foreground">Confirmez avec votre mot de passe actuel. Un email de confirmation sera envoyé à la nouvelle adresse.</p>
        <div>
