...
import { LayoutDashboard, Package, ShoppingCart, Boxes, Settings, LogOut, ChevronLeft, Menu, Globe, Bell, X } from "lucide-react";
import { cn } from "@/lib/utils";
import { supabase } from "@/integrations/supabase/client";
import { useLang } from "@/context/LanguageContext";
import { formatPrice } from "@/types/database";
import { motion, AnimatePresence } from "framer-motion";
import { startAdminTranslation, stopAdminTranslation, restoreAdminTranslation } from "@/lib/adminI18n";
...
export default function AdminLayout() {
  ...
  // Préchargement des chunks admin pour rendre la navigation entre onglets instantanée
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      // Ne rediriger que sur déconnexion explicite, pas sur les refresh de token / USER_UPDATED
  ...
  useEffect(() => {
    ...
    if (!root) return;
    if (lang === "ar") {
      const id = requestAnimationFrame(() => startAdminTranslation(root));
      return () => { cancelAnimationFrame(id); stopAdminTranslation(); restoreAdminTranslation(); };
    } else {
      stopAdminTranslation();
      restoreAdminTranslation();
    }
  }, [lang, location.pathname, loading]);
  ...
  return (
    <div ref={adminRootRef} dir={lang === "ar" ? "rtl" : "ltr"} className="min-h-screen bg-background flex">
