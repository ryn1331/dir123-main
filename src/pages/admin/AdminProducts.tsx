...
import { compressImage, compressProductPhoto } from "@/lib/imageUtils";
...
export default function AdminProducts() {
  ...
  const patchProductCache = (id: string, patch: Partial<DbProduct>) => {
    queryClient.setQueryData<DbProduct[]>(["admin-products"], (old) =>
      (old || []).map((p) => (p.id === id ? { ...p, ...patch } : p))
    );
  };
  const { data: products = [], refetch: loadProducts } = useQuery<DbProduct[]>({
    queryKey: ["admin-products"],
    queryFn: async () => {
      const { data } = await supabase.from("products").select("id,name,brand,category,price,old_price,image_url,gallery,flavors,weights,description,nutrition_facts,in_stock,is_top_sale,is_promo,rating,reviews_count,stock_qty,usage_instructions,conseils,expiration_date,objectives,created_at,updated_at,format").order("created_at", { ascending: false });
  ...
  const openEdit = (p: DbProduct) => {
    ...
    setForm({
      ...
      flavors: p.flavors || [], weights: p.weights || [], format: (p as any).format || "",
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      // Optimistic: affiche immediatement le nouveau stock
      patchProductCache(editing.id, data);
      const { error } = await supabase.from("products").update(data).eq("id", editing.id);
      if (error) { toast.error("Erreur"); setSaving(false); loadProducts(); return; }
  ...
  const handleDelete = async (id: string) => {
    ...
    queryClient.setQueryData<DbProduct[]>(["admin-products"], (old) => (old || []).filter((p) => p.id !== id));
    const { error } = await supabase.from("products").delete().eq("id", id);
    if (error) { toast.error("Erreur"); loadProducts(); return; }
  ...
  const handleStockChange = async () => {
    ...
    const newQty = stockMode === "add"
      ? currentQty + stockDeduct
      : Math.max(0, currentQty - stockDeduct);
    const newInStock = newQty > 0;
    // Optimistic update du cache: l'affichage change instantanement
    patchProductCache(showStockModal.id, { stock_qty: newQty, in_stock: newInStock } as any);
    const { error } = await supabase.from("products").update({ stock_qty: newQty, in_stock: newInStock }).eq("id", showStockModal.id);
    if (error) { toast.error("Erreur"); loadProducts(); return; }
    toast.success(stockMode === "add"
      ? `✅ ${stockDeduct} unité(s) ajoutée(s) — Stock: ${newQty}`
      : `✅ ${stockDeduct} unité(s) retirée(s) — Stock: ${newQty}`);
      ...
      {showStockModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-xl p-5 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-xs text-muted-foreground mb-3">{showStockModal.name} — Stock actuel: <strong>{(showStockModal as any).stock_qty ?? 0}</strong></p>
            <div className="grid grid-cols-2 gap-1.5 mb-3 p-1 bg-secondary rounded-lg">
              <button onClick={() => setStockMode("add")} className={`h-8 rounded-md text-xs font-medium transition ${stockMode === "add" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}>+ Ajouter</button>
              <button onClick={() => setStockMode("deduct")} className={`h-8 rounded-md text-xs font-medium transition ${stockMode === "deduct" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}>− Retirer</button>
            </div>
            <label className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1 block">
              {stockMode === "add" ? "Quantité ajoutée" : "Quantité vendue"}
            </label>
            <div className="flex items-center gap-2 mb-4">
              ...
              <input type="number" value={stockDeduct} onChange={e => setStockDeduct(Math.max(1, +e.target.value))} min={1} className="w-20 h-9 rounded-lg bg-secondary border border-border text-center text-sm" />
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleStockChange} className="flex-1 h-9 rounded-lg gradient-primary text-primary-foreground text-xs font-bold">
                {stockMode === "add" ? "✅ Ajouter au stock" : "✅ Retirer du stock"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center p-2 pt-8 md:items-center md:p-4">
          <div className="bg-card border border-border rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4 space-y-4">
              ...
              {/* Format / Conditionnement — pour compléments alimentaires */}
              <FormField
                label="Format / Conditionnement"
                value={form.format}
                onChange={v => setForm(f => ({ ...f, format: String(v) }))}
                placeholder="ex: 60 gélules · 30 portions · 500 g · 1 mois de cure"
              />
                ...
                <div className="p-2.5">
                  ...
                  <div className="flex items-center justify-between mt-1.5">
                    ...
                    <div className="flex gap-0.5">
                      ...
                      <button onClick={() => { setShowStockModal(p); setStockMode("add"); setStockDeduct(1); }} className="p-1 rounded hover:bg-secondary text-muted-foreground hover:text-emerald-400" title="Réapprovisionner"><Plus size={11} /></button>
                      <button onClick={() => { setShowStockModal(p); setStockMode("deduct"); setStockDeduct(1); }} className="p-1 rounded hover:bg-secondary text-muted-foreground hover:text-orange-400" title="Vente présentiel"><Minus size={11} /></button>
                    ...
                    <tr key={p.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex items-center justify-end gap-0.5">
                          ...
                          <button onClick={() => { setShowStockModal(p); setStockMode("add"); setStockDeduct(1); }} className="p-1.5 rounded hover:bg-secondary text-muted-foreground hover:text-emerald-400 transition-colors" title="Réapprovisionner"><Plus size={13} /></button>
                          <button onClick={() => { setShowStockModal(p); setStockMode("deduct"); setStockDeduct(1); }} className="p-1.5 rounded hover:bg-secondary text-muted-foreground hover:text-orange-400 transition-colors" title="Vente présentiel"><Minus size={13} /></button>
