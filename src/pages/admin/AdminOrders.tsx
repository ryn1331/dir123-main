...
// Stock management: adjust stock atomically via DB function (prevents race conditions)
async function adjustStock(orderItems: any[], direction: "decrement" | "increment") {
  await Promise.all(orderItems.map(async (item) => {
    ...
    // Atomic operation: decrement uses GREATEST(0, ...) in DB function,
    // increment passes negative quantity to add stock back.
    const qty = direction === "decrement" ? item.quantity : -item.quantity;
...
export default function AdminOrders() {
  ...
  const { data: orders = [], refetch: loadOrders } = useQuery<any[]>({
    ...
    queryFn: async () => {
      ...
      return (ordersData || []).map(o => ({ ...o, _items: itemsByOrder[o.id] || [] }));
    },
    staleTime: 30_000,
  ...
  const updateStatus = async (orderId: string, newStatus: string) => {
    ...
    if (selectedOrder?.id === orderId) {
      ...
      if (refreshed.data) setSelectedOrder((prev: any) => ({ ...prev, ...refreshed.data }));
      else setSelectedOrder((prev: any) => ({ ...prev, status: newStatus }));
    }
  };
