...
export default function AdminDashboard() {
  ...
  const { data: dash } = useQuery({
    ...
    queryFn: async () => {
      ...
      const lowStockProducts = products.filter(p => (p.stock_qty ?? 0) <= 5).sort((a, b) => (a.stock_qty ?? 0) - (b.stock_qty ?? 0)).slice(0, 5);
      ...
      const ordersByDay = Object.entries(dayMap).map(([date, data]) => {
      ...
      const topProducts = Object.entries(productMap).map(([name, data]) => ({ name, ...data })).sort((a, b) => b.revenue - a.revenue).slice(0, 5);

      return { stats, recentOrders, topProducts, categoryData, ordersByDay, statusData, lowStockProducts, expiringProducts };
    },
    staleTime: 60_000,
    refetchOnWindowFocus: false,
  });

  const stats = dash?.stats ?? { revenue: 0, orders: 0, products: 0, clients: 0, pendingOrders: 0, avgOrderValue: 0, deliveredOrders: 0, confirmedOrders: 0, returnOrders: 0 };
  const recentOrders = dash?.recentOrders ?? [];
  const topProducts = dash?.topProducts ?? [];
  const categoryData = dash?.categoryData ?? [];
  const ordersByDay = dash?.ordersByDay ?? [];
  const statusData = dash?.statusData ?? [];
  const lowStockProducts = dash?.lowStockProducts ?? [];
  const expiringProducts = dash?.expiringProducts ?? [];
