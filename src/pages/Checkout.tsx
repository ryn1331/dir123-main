...
export default function Checkout() {
  ...
  const deliveryOptions = useMemo(() => selectedZone ? getDeliveryOptionsFromZone(selectedZone, {
    yalidineHasStopDesk: selectedCommune?.has_stop_desk === true,
    // ZR: bureau seulement si la wilaya a un hub ET la commune est liee (sinon force domicile)
    zrHasStopDesk: !!selectedZone.zr_hub_id && selectedCommune?.zr_has_bureau === true,
  }) : [], [selectedZone, selectedCommune]);
  ...
  const handleWilayaChange = (w: string) => {
    setForm(f => ({ ...f, wilaya: w, commune: "", delivery: "" }));
  };

  const handleCommuneChange = (commune: string) => {
    setForm(f => ({ ...f, commune, delivery: "" }));
  };

  useEffect(() => {
    if (!selectedZone?.code) { setCommunes([]); return; }
    let cancelled = false;
    setLoadingCommunes(true);
    (async () => {
      try {
        const url = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/shipping-locations?service=yalidine&type=communes&wilaya_id=${selectedZone.code}`;
        const [yResp, zrRows] = await Promise.all([
          fetch(url, { headers: { Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY}`, apikey: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY } }),
          supabase.from("delivery_communes").select("name,zr_territory_id").eq("wilaya_code", selectedZone.code),
        ]);
        const yJson = await yResp.json();
        if (cancelled) return;
        const norm = (s: string) => (s || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, "");
        const zrMap = new Map<string, boolean>();
        (zrRows.data || []).forEach((r: any) => zrMap.set(norm(r.name), !!r.zr_territory_id));
        if (yResp.ok) {
          const merged = (yJson as DeliveryCommune[]).map((c) => ({ ...c, zr_has_bureau: zrMap.get(norm(c.name)) === true }));
          setCommunes(merged);
        }
      } catch (_) {
  ...
  useEffect(() => {
    if (step !== 2 || !deliveryOptions.length) return;
    if (!form.delivery || !deliveryOptions.some(o => o.id === form.delivery)) {
      setForm(f => ({ ...f, delivery: deliveryOptions[0]?.id || "" }));
    }
  }, [step, deliveryOptions, form.delivery]);
    ...
    <div className="container py-6 md:py-10 min-h-screen max-w-2xl mx-auto pb-24">
      ...
      {step === 4 && orderResult ? (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-10">
          ...
          <h2 className="font-heading text-2xl font-bold mb-2">{t("checkout.orderCmd")} {orderResult.number} {t("checkout.orderReceived")}</h2>
          <p className="text-muted-foreground mb-6 max-w-sm mx-auto">{t("checkout.adminWillCall")}</p>
          <div className="max-w-sm mx-auto p-4 rounded-xl bg-secondary/50 border border-border text-sm space-y-1 mb-6 text-left">
            ...
            <p className="font-heading font-bold"><span className="text-muted-foreground font-normal">{t("of.total")}:</span> {formatPrice(orderResult.total)}</p>
          </div>
          <div className="flex flex-col gap-3 max-w-sm mx-auto">
            <Button asChild className="h-11 bg-[#1877F2] hover:bg-[#166FE5] text-white rounded-xl font-heading">
              <a href={MESSENGER_URL} target="_blank" rel="noopener noreferrer">
                <MessageCircle size={18} className="me-2" /> {t("checkout.contactMessenger")}
              </a>
            </Button>
            <Button variant="outline" onClick={() => navigate("/catalogue")} className="h-11 rounded-xl">{t("checkout.continueShopping")}</Button>
          </div>
        </motion.div>
      ) : (
            ...
            <AnimatePresence mode="wait">
              {step === 1 && (
                <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
                  ...
                  <div>
                    <label htmlFor="ck-phone" className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1 block">{t("checkout.phoneLabelField")} *</label>
                  ...
                  <div>
                    <label htmlFor="ck-wilaya" className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1 block">{t("checkout.wilaya")} *</label>
                    <div className="relative">
                  ...
                  <div>
                    <label htmlFor="ck-commune" className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1 block">{t("checkout.communeField")} *</label>
                    {loadingCommunes ? (
                      <div className="field-input flex items-center text-muted-foreground"><Loader2 size={14} className="animate-spin me-2" /> {t("common.loading")}</div>
                    ) : communes.length > 0 ? (
                      <select id="ck-commune" value={form.commune} onChange={e => handleCommuneChange(e.target.value)} className="field-input appearance-none pr-8" autoComplete="address-level2">
                        <option value="">{t("common.choose")}</option>
                        {communes.map(c => <option key={c.id} value={c.name}>{c.name}{c.has_stop_desk ? " 📦" : ""}</option>)}
                      </select>
                    ) : (
                      <input id="ck-commune" placeholder={t("checkout.commune")} value={form.commune} onChange={e => handleCommuneChange(e.target.value)} className="field-input" maxLength={100} autoComplete="address-level2" />
                    )}
                  </div>
                  <div>
                    <label htmlFor="ck-address" className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1 block">{t("checkout.addressField")} *</label>
                  ...
                  <div>
                    <label htmlFor="ck-notes" className="text-[10px] text-muted-foreground mb-1 block">{t("checkout.noteOptional")}</label>
                    ...
                    <button key={opt.id} onClick={() => update("delivery", opt.id)} className={`w-full p-4 rounded-xl border text-left transition-all ${form.delivery === opt.id ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/30"}`}>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">{opt.type === "bureau" ? t("checkout.deliveryBureauDesc") : t("checkout.deliveryHomeDesc")}</p>
                        </div>
            ...
            <div className="space-y-3 mb-4">
              {items.map(item => (
                <div key={`${item.productId}-${item.flavor}-${item.weight}`} className="flex gap-2">
                  <img src={getStorageUrl(item.imageUrl, 80)} alt={item.name} className="w-12 h-12 rounded-xl object-cover" width={48} height={48} />
