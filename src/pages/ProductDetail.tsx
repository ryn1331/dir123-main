...
import { ShoppingCart, ChevronLeft, Check, Loader2, Truck, Phone, AlertCircle } from "lucide-react";
      ...
      <div className="container py-4 md:py-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-14">
          ...
          <div className="flex flex-col">
            ...
            <h1 dir="auto" className="font-heading text-2xl md:text-4xl font-bold mt-1 mb-3 text-foreground leading-tight bidi-auto">{product.name}</h1>

            {(product as any).format && (
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-secondary/60 border border-border/50 mb-5">
                <span className="text-[10px] font-body uppercase tracking-[0.12em] text-muted-foreground">Format</span>
                <span className="text-xs font-body font-medium text-foreground" dir="auto">{(product as any).format}</span>
              </div>
            )}

            <div className="flex items-baseline gap-3 mb-6">
              <span className="font-heading font-bold text-3xl text-red-600">{formatPrice(product.price)}</span>
              {product.old_price && <span className="text-lg text-muted-foreground line-through">{formatPrice(product.old_price)}</span>}
            </div>

            <p dir="auto" className="text-sm text-muted-foreground mb-6 leading-relaxed font-body bidi-auto">{product.description}</p>
            ...
            {(() => {
              ...
              if (isOutOfStock) {
                return (
                  <>
                    <div role="alert" className="mb-5 rounded-2xl border border-destructive/30 bg-destructive/5 p-4 flex gap-3">
                      <AlertCircle size={20} className="text-destructive shrink-0 mt-0.5" />
                      <div className="flex-1">
                        <p className="font-heading font-semibold text-sm text-destructive mb-1">{t("product.unavailableTitle")}</p>
                        <p className="text-xs text-muted-foreground leading-relaxed" dir="auto">{t("product.unavailableDesc")}</p>
                      </div>
                    </div>
                    <div className="flex gap-3 mb-6">
                      <Button disabled className="flex-1 h-12 font-body text-sm font-semibold rounded-full opacity-60 cursor-not-allowed">
              ...
              return (
                <>
                  <div className="flex items-center gap-1.5 text-sm mb-6">
                    ...
                    <span className="text-primary font-medium">{t("product.inStock")}</span>
                  </div>
                  <div className="flex gap-3 mb-6">
                    <Button
          ...
          <div className="py-8">
            {activeTab === "description" && (
              <div className="prose prose-sm max-w-none text-muted-foreground font-body">
                <p dir="auto" className="bidi-auto">{product.description || t("product.descriptionTBD")}</p>
            ...
            {activeTab === "usage" && (
              <div className="prose prose-sm max-w-none text-muted-foreground font-body">
                {usageInstructions ? (
                  <p dir="auto" className="whitespace-pre-line bidi-auto">{usageInstructions}</p>
                ) : (
            ...
            {activeTab === "conseils" && (
              <div className="prose prose-sm max-w-none text-muted-foreground font-body">
                {conseils ? (
                  <p dir="auto" className="whitespace-pre-line bidi-auto">{conseils}</p>
                ) : (
