...
const Catalog = React.forwardRef<HTMLDivElement>(function Catalog(_, ref) {
  ...
  const fetchProducts = useCallback(async (offset: number, append: boolean) => {
    ...
    let query = supabase.from("products_public")
      .select("id,name,brand,category,price,old_price,image_url,rating,reviews_count,is_promo,is_top_sale,in_stock,objectives")
