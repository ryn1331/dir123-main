...
interface ZrCommune { id: string | number; name: string; code?: string; has_stop_desk?: boolean; }
...
export default function OrderForm({ product, quantity, onClose }: OrderFormProps) {
    ...
    (async () => {
      try {
        ...
        if (service === "yalidine") {
          ...
          const all = Object.values(fees.per_commune || {});
          const norm = (s: string) => s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, "");
          const match = form.communeName
            ? all.find(c => norm(c.commune_name) === norm(form.communeName)) || all[0]
            : all[0];
  ...
  const selectedYalidineCommune = service === "yalidine" ? yalidineCommunes.find(c => c.name === form.communeName) : null;
  const selectedZrCommune = service === "zr" ? zrCommunes.find(c => c.name === form.communeName) : null;
  const selectedZone = fallbackZones.find(z => z.code === form.wilayaId || z.name === form.wilayaName);
  const canUseOffice =
    service === "yalidine" ? selectedYalidineCommune?.has_stop_desk === true :
    // ZR: bureau seulement si la wilaya a un hub ET la commune est liee (sinon force domicile)
    service === "zr" ? (!!selectedZone?.zr_hub_id && selectedZrCommune?.has_stop_desk === true) :
    true;
  const subtotal = product.price * form.qty;
  const total = subtotal + deliveryFee;

  const missing: string[] = [];
  if (!service) missing.push(t("of.deliveryService").toLowerCase());
  if (form.name.trim().length < 2) missing.push(t("of.fullName").toLowerCase());
  if (form.phone.trim().length < 10) missing.push(t("of.phone").toLowerCase());
  if (!form.wilayaName) missing.push(t("checkout.wilaya").toLowerCase());
  if (!form.communeName.trim()) missing.push(t("checkout.communeField").toLowerCase());
  if (form.address.trim().length > 0 && form.address.trim().length < 3) missing.push(t("checkout.addressField").toLowerCase());
  if (!form.deliveryType) missing.push(t("of.deliveryType").toLowerCase());
  else if (deliveryFee <= 0) missing.push("—");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err?.message || t("checkout.sendError"));
      console.error(err);
    } finally {
      setLoading(false);
    }
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 300 }}
        ...
        <div className="shrink-0 bg-background/90 backdrop-blur-md border-b border-border px-4 py-3 flex items-center justify-between">
          <h2 className="font-heading text-base font-bold text-foreground">{t("of.title")}</h2>
          <button onClick={onClose} className="p-1.5 rounded-full hover:bg-secondary text-muted-foreground" aria-label={t("common.close")}><X size={16} /></button>
        </div>
        ...
        {step === "form" ? (
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto flex flex-col" noValidate>
            <div className="p-4 space-y-3 flex-1">
              ...
              <fieldset>
                <legend className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1.5">1. {t("of.deliveryService")} *</legend>
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setService("yalidine")}
                    ...
                    <p className="text-[9px] text-muted-foreground mt-0.5">{t("of.nationalCoverage")}</p>
                  </button>
                  <button type="button" onClick={() => setService("zr")}
                    ...
                    <p className="text-[9px] text-muted-foreground mt-0.5">{t("of.fastDelivery")}</p>
                  </button>
                </div>
              </fieldset>
              ...
              {service && (
                <>
                  <div>
                    <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1 block">{t("of.fullName")} *</label>
                  ...
                  <div>
                    <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1 block">{t("of.phone")} *</label>
                  ...
                  <div>
                    <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1 block">2. {t("checkout.wilaya")} *</label>
                    {loadingLocations ? (
                      <div className="h-9 rounded-lg bg-secondary border border-border flex items-center px-3 text-xs text-muted-foreground">
                        <Loader2 size={12} className="animate-spin me-2" /> {t("common.loading")}
                      </div>
                    ) : (
                  ...
                  {/* Commune */}
                  <div>
                    <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1 block">{t("checkout.communeField")} *</label>
                    {service === "yalidine" && yalidineCommunes.length > 0 ? (
                        <select value={form.communeName} onChange={e => setForm(f => ({ ...f, communeName: e.target.value, deliveryType: "" }))}
                        className="w-full h-9 rounded-lg bg-secondary border border-border px-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary" required>
                      ...
                      <input type="text" value={form.communeName} onChange={e => setForm(f => ({ ...f, communeName: e.target.value }))}
                        placeholder={t("checkout.communePlaceholder")} className="w-full h-9 rounded-lg bg-secondary border border-border px-3 text-sm focus:outline-none focus:ring-1 focus:ring-primary" required maxLength={100} />
                    )}
                  </div>
                  ...
                  {form.wilayaId && (
                    <fieldset>
                      <legend className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1">
                        3. {t("of.deliveryType")} {loadingRates && <Loader2 size={10} className="inline animate-spin ms-1" />}
                      </legend>
                      <div className="space-y-1.5">
                        {rates && rates.home > 0 && (
                          <button type="button" onClick={() => setForm(f => ({ ...f, deliveryType: "domicile" }))}
                            ...
                            <div className="flex items-center justify-between">
                              <span className="font-medium">🏠 {t("of.home")}</span>
                        ...
                        {rates && rates.desk > 0 && canUseOffice && (
                          <button type="button" onClick={() => setForm(f => ({ ...f, deliveryType: "bureau" }))}
                            ...
                            <div className="flex items-center justify-between">
                              <span className="font-medium">📦 {t("of.office")}</span>
                              <div className="flex items-center gap-1.5">
                        ...
                        {!loadingRates && !rates && form.communeName && (
                          <p className="text-[10px] text-amber-500">{t("of.noRate")}</p>
                        )}
                      </div>
                    </fieldset>
                  )}
                  ...
                  <div>
                    <label className="text-[10px] text-muted-foreground mb-1 block">{t("checkout.noteOptional")}</label>
                    <input value={form.comment} onChange={e => setForm(f => ({ ...f, comment: e.target.value }))} className="w-full h-9 rounded-lg bg-secondary border border-border px-3 text-sm focus:outline-none focus:ring-1 focus:ring-primary" maxLength={500} placeholder={t("checkout.notesPlaceholder")} />
                  </div>
                </>
              )}
            </div>
            ...
            <div className="shrink-0 border-t border-border bg-background/95 backdrop-blur-md p-4 space-y-2.5">
              <div className="flex items-center justify-between text-xs"><span className="text-muted-foreground">{t("of.subtotal")}</span><span>{formatPrice(subtotal)}</span></div>
              <div className="flex items-center justify-between text-xs"><span className="text-muted-foreground">{t("of.delivery")}</span><span>{deliveryFee > 0 ? formatPrice(deliveryFee) : "—"}</span></div>
              <div className="flex items-center justify-between font-heading font-bold text-sm pt-1.5 border-t border-border"><span>{t("of.total")}</span><span className="text-primary">{formatPrice(total)}</span></div>
              <Button type="submit" disabled={loading || !isValid} className="w-full h-11 font-heading text-sm bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl shadow-lg shadow-primary/20 disabled:opacity-50">
                {loading ? <><Loader2 size={14} className="animate-spin me-2" /> {t("common.send")}</> : `✅ ${t("of.confirm")} · ${formatPrice(total)}`}
              </Button>
              <p className="text-[9px] text-center text-muted-foreground">💳 {t("of.cod")}</p>
              {!isValid && missing.length > 0 && (
                <p className="text-[10px] text-center text-amber-500">{t("common.missing")}: {missing.join(", ")}</p>
              )}
            </div>
          </form>
        ) : (
          <div className="flex-1 overflow-y-auto p-5 flex flex-col items-center justify-center">
            ...
            <h3 className="font-heading text-xl font-bold text-foreground mb-2">{t("checkout.orderCmd")} {orderNumber} {t("checkout.orderReceived")}</h3>
            <p className="text-xs text-muted-foreground mb-4 max-w-xs text-center">{t("checkout.adminWillCall")}</p>
            <div className="w-full p-3 rounded-lg bg-secondary/50 border border-border text-left text-xs space-y-1 mb-4">
              <p><span className="text-muted-foreground">{t("of.product")}:</span> {product.name} × {form.qty}</p>
              <p><span className="text-muted-foreground">{t("of.service")}:</span> {service === "yalidine" ? "Yalidine" : "ZR Express"} {form.deliveryType === "bureau" ? t("of.office") : t("of.home")}</p>
              <p className="font-heading font-bold pt-1 border-t border-border"><span className="text-muted-foreground font-normal">{t("of.total")}:</span> {formatPrice(total)}</p>
            </div>
            <div className="flex flex-col gap-2.5 w-full">
              ...
              <Button variant="outline" onClick={onClose} className="h-10 rounded-xl border-border text-sm">{t("checkout.continueShopping")}</Button>
            </div>
          </div>
        )}
      </motion.div>
