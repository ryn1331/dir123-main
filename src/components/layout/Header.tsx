  ...
  return (
    <header className="sticky top-0 z-50 bg-background/90 backdrop-blur-xl border-b border-border/50" role="banner">
      ...
      <div className="container flex items-center justify-between h-14 md:h-16">
        <Link to="/" className="flex items-center gap-2" aria-label={`Dir l'Affaire - ${t("a11y.home")}`}>
          <span className="font-heading text-lg md:text-xl font-bold tracking-tight text-foreground">
            Dir l'Affaire
          </span>
        </Link>

        <nav className="hidden md:flex items-center gap-6 text-sm font-medium" aria-label={t("a11y.mainNav")}>
          <Link to="/catalogue?univers=beaute" className="text-muted-foreground hover:text-foreground transition-colors">{t("nav.beaute")}</Link>
          <Link to="/catalogue?univers=sante" className="text-muted-foreground hover:text-foreground transition-colors">{t("nav.sante")}</Link>
        </nav>
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            aria-label={`${t("a11y.cart")} (${itemCount})`}
          >
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            <nav className="container py-4 flex flex-col gap-0.5" aria-label={t("a11y.mobileNav")}>
              {[
                { to: "/catalogue?univers=beaute", label: t("universe.beaute.title") },
                { to: "/catalogue?univers=sante", label: t("universe.sante.title") },
              ].map(link => (
