...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Source unique : table delivery_zones (wilayas) + delivery_communes (communes)
    const supabaseUrl = Deno.env.get("SUPABASE_URL")!;
    const serviceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") || Deno.env.get("SUPABASE_ANON_KEY")!;
    const sb = createClient(supabaseUrl, serviceKey);

    if (type === "wilayas") {
      const col = service === "yalidine" ? "has_yalidine" : "has_zr_express";
      ...
      const out = (zones || []).map((z: any) => ({
        ...
        name: z.name,
        code: z.code,
        territory_id: ZR_WILAYA_UUID_BY_CODE[z.code],
      }));
    ...
    // type === "communes"
    if (!wilayaId) return jsonResponse([]);
    const code = String(wilayaId).padStart(2, "0");
    const { data: communes } = await sb
      .from("delivery_communes")
      .select("id,name,zr_territory_id")
      .eq("wilaya_code", code)
      .order("name");

    let stopdeskCommunes = new Set<string>();
    let zrHasHub = false;
    if (service === "yalidine") {
      try {
        ...
        if (apiId && apiToken) {
          const resp = await fetch(`${YALIDINE_BASE}/centers/?wilaya_id=${parseInt(code, 10)}`, {
            headers: { "X-API-ID": apiId, "X-API-TOKEN": apiToken },
          });
          if (resp.ok) {
            const centersJson = await resp.json();
            const centers = Array.isArray(centersJson.data) ? centersJson.data : Array.isArray(centersJson) ? centersJson : [];
            stopdeskCommunes = new Set(centers.map((c: any) => normName(c.commune_name || "")).filter(Boolean));
          } else {
            console.error("Yalidine centers error:", resp.status, (await resp.text()).slice(0, 200));
          }
        }
      } catch (e) {
        console.error("Yalidine centers lookup failed:", e);
      }
    } else if (service === "zr") {
      // ZR: bureau dispo si la wilaya a un hub pickup configure
      const { data: zone } = await sb
        .from("delivery_zones")
        .select("zr_hub_id")
        .eq("code", code)
        .maybeSingle();
      zrHasHub = !!zone?.zr_hub_id;
    }

    const out = (communes || []).map((c: any) => ({
      id: c.name,
      name: c.name,
      code,
      // ZR: bureau seulement si la wilaya a un hub ET la commune est liee a un territoire ZR
      has_stop_desk: service === "yalidine"
        ? stopdeskCommunes.has(normName(c.name))
        : (zrHasHub && !!c.zr_territory_id),
    }));
    setCache(cacheKey, out);
    return jsonResponse(out);
  } catch (err) {
